...
/**
 * The client object manages interactions with the user interfase (UI)
 * to create, read, update and delete notebook and notes.
 ...
 * @property {Object} notebook      -   Functions for managing notes in the UI
 */
export const client = {
   notebook: {
      /**
       * Creates a new notebook in the UI, based in provided notebook data
       ...
       *
       * @param {Array<Objetc>} notebookList    -   List of notebook data to display
       */
      read(notebookList) {
         disableNoteCreateBtns(notebookList.length);
         notebookList.forEach((notebookData, index) => {
            const /**{HTLMElement} */ $navItem = NavItem(
                  notebookData.id,
                  notebookData.name
               );
      ...
      delete(notebookID) {
         ...
         const /**{HTMLElement} */ $activeNavItem =
   ...
   note: {
      ...
      create(noteData) {
          ...
          */
         if (!$notePanel.querySelector("[data-note-panel]"))
            $notePanel.innerHTML = "";
       ...
       *
       * @param {string} noteId           -  ID od the note to delete
